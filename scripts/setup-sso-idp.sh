...
if [[ ${CLUSTER_ID} ]]; then
  ...
  SECRET_NAME="idp-$REALM"
  oc delete secret $SECRET_NAME -n openshift-config 2>/dev/null || true
  oc create secret generic $SECRET_NAME --from-literal=clientSecret=$CLIENT_SECRET -n openshift-config
